...
- 로컬 PC(외부 인터넷 환경)에서 브라우저 및 `curl`로 퍼블릭 IP 호출 시 무한 로딩 후 요청이 실패함.
  ```bash
  $ curl -i http://3.35.135.178/health
...
## 2. 가설 수립 (Hypothesis)
1. **가설 1**: EC2 인스턴스에 적용된 보안 그룹(Security Group) 인바운드 규칙에서 HTTP(80) 포트 트래픽이 허용되지 않았을 가능성.
2. **가설 2**: Nginx 서비스가 `80` 포트에서 외부 인터페이스(`0.0.0.0:80`)를 바인딩하지 않고 `127.0.0.1:80`으로만 수신 대기하고 있을 가능성.

---

## 3. 검증 (Verification)
- **가설 2 검증**: EC2 내부에서 포트 바인딩 상태 확인
  ```bash
  $ sudo ss -tulpn | grep :80
  tcp   LISTEN 0      511          0.0.0.0:80        0.0.0.0:*    users:(("nginx",pid=...,fd=...))
  ```
  → Nginx는 `0.0.0.0:80` 전체 인터페이스에서 정상 수신 중이므로 서버 내부 문제는 아님.
- **가설 1 검증**: EC2에 연결된 보안 그룹(`web-sg`) 인바운드 규칙 점검
